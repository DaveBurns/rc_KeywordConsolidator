...
local rcMetadataSignature = 'com.robcole.jpeg.metadata' -- this will be preceded by FF E7 {len1} {len2}
...
--[[
        Synopsis:       Read length word.
        
        Notes:          Could have rc-io-utils that does this.###4
--]]
local function _getDoubleByteNumber( file )
    local byte1, byte2
    local word
    byte1 = _getByte( file )
    if byte1 == nil then
    ...
    word = (byte1 * 256) + byte2 -- reminder: jpeg is big-endian on all platforms/architectures.
    return word
end
...
--]]
function RcJpegUtils.readFileWithMetadata( filePath, metadataSignature, blockType )
    assert( RcFileUtils.existsAsFile( filePath ), "read-file valid for pre-existing files only, not this: " .. RcLua.toString( filePath ) )
    local contents, qualification = RcFileUtils.readFile( filePath ) -- reads contents regardless of test mode and returns same.
    if contents then
        -- good
    else
    ...
    repeat
        ...
        if start then
            ...
            if start > 6 then -- may be legal.
                local blockStartChar = RcString.getChar( contents, start - 4 )
                local blockTypeMarker = RcString.getChar( contents, start - 3 )
                local blockLen1 = RcString.getChar( contents, start - 2 )
                local blockLen2 = RcString.getChar( contents, start - 1 )
                if blockStartChar == string.char( 0xFF ) then
                    if blockTypeMarker == string.char( blockType ) then -- if I ever change the block type written, I'll need to change block type detection.###5
                        ...
                        if RcUtils.debugMode then
                            RcUtils.logMessageLine( "Got RC Metadata: " .. metadata )
                        end
                        postamble = string.sub( contents, stop + dataLength + 1 )
                        break                        
                    else
                        if RcUtils.debugMode then
                            RcUtils.logWarning( "Possibly corrupt jpeg file: " .. filePath .. " - rc-meta-data signature found but bad block-type: " .. RcLua.toString( blockTypeMarker ) )
                        end
                    end
                else
                    if RcUtils.debugMode then
                        RcUtils.logWarning( "Possibly corrupt jpeg file: " .. filePath .. " - rc-meta-data signature found but block start indicator missing, signature offset: " .. RcLua.toString( start ) )
                    end
                end
            else
                if RcUtils.debugMode then
                    RcUtils.logWarning( "Possibly corrupt jpeg file: " .. filePath .. " - rc-meta-data signature found but too early, offset: " .. RcLua.toString( blockTypeMarker ) )
                end
            end
        else
            --[[if RcUtils.debugMode then
                -- RcUtils.logWarning( "No RC Metadata signature in " .. filePath .. ", start: " .. RcLua.toString( start) .. ", sig: " .. RcLua.toString( metadataSignature ) .. ", len: " .. string.len( contents ) )
            end--]]
            break
        end
...
--[[
        Synopsis:       Read a jpeg file in such a way that it identifies rc-jpeg-metadata,
                        and can be re-written with new or modified rc-jpeg-metadata with impunity.
                        
        Motivation:     To support rc-jpeg-metadata, namely photo-uuid for looking up source photo
        ...
        Initial Application:    Metadata to contain photo-id & exact date/time of last source mod to support corelation of target to catalog database and change determination.
                        
        Returns:        preamble, rc-jpeg-metadata - undressed, postamble, nil -- worked: metadata nil if not present.
                        nil, nil, nil, qualification -- failed.
--]]
function RcJpegUtils.readFileWithRcMetadata( filePath )
	return RcJpegUtils.readFileWithMetadata( filePath, rcMetadataSignature, rcBlockType )
end
...
--[[
        ...
        ###2:               - this should really be in with the xmp-core functions, along with other xmp related functions.
...
--]]                            
function RcJpegUtils.parseJpegFileToPieces( contents )
	local startAt = 1
	...
	if setStart then -- found settings, and have pointers to last set
    	...
    	repeat
            ...
            if p1 then
                ...
                len1 = string.byte( RcString.getChar( contents, p1 + 2 ) )
                len2 = string.byte( RcString.getChar( contents, p1 + 3 ) )
                local len = (len1 * 256) + len2
                local testChar = RcString.getChar( contents, p1 + 2 + len )
                if testChar == string.char( 0xFF ) then -- presumably we've got a legal app1 block.
                    ...
                    if start then
                        ...
                        if start == setStart and setStop < p1 + 2 + len then
                            ...
                            -- _debugTrace( "sblklen458", "p1: " .. RcLua.toString( p1 ) )
                            -- _debugTrace( "setstops", "setstart: " .. RcLua.toString( setStart ) .. ", setstop: " .. RcLua.toString( setStop ) )
                    	    ...
                    	    local _dt = LOC( "$$$/X=jpre length: ^1, sbh: ^2, spre: ^3, ss: ^4, spost: ^5, jpost: ^6", RcLua.toString( jpreLen ), RcLua.toString( sblkHLen ), RcLua.toString( spreLen ), RcLua.toString( ssLen ), RcLua.toString( spstLen ), RcLua.toString( jpstLen ) )
                    	    ...
                    	    if testLen == contentLen then
                    	        -- good
                    		    -- _debugTrace( "rctpudifdfekl", "test len match" )
                    		    return jpegPreamble, settingsBlockHeader, settingsPreamble, settingsString, settingsPostamble, jpegPostamble, nil
                    		else
                    		    -- local diffLen, s1rem, s2rem = RcString.getDiff( testContents, contents )
                    		    -- _debugTrace( "rctpudifl", LOC( "$$$/X=Diff Len: ^1, s1rem: ^2, s2rem: ^3", diffLen, s1rem, s2rem) )
                    		    LrErrors.throwUserError( "Unable to parse jpeg file to pieces - please report problem (length mismatch)." )
                    		end
                        else -- potential settings are not target settings.
                            -- _debugTrace( "asdf570", "not our settings" )
                        end
                    else
                        -- _debugTrace( "rcjpunopotset", "no potential settings in block" )
                    end
   					startAt = p1 + 2 + len
                else
...
--[[
		Synopsis:			Reads the contents of specified file, searches for the block containing xmp/camera-raw-settings, and returns
...
--]]
function RcJpegUtils.readFileWithXmpMetadata( filePath )
    local oLen
	local contents, orNot = RcFileUtils.readFile( filePath )
	if contents then
		-- good
	...
	local jpre, blkhdr, spre, settingsString, spst, jpst, qual = RcJpegUtils.parseJpegFileToPieces( contents )
...
function RcJpegUtils.reWriteFileWithMetadata( filePath, preamble, metadata, postamble, metadataSignature, blockType )
    local blockLength = string.len( metadataSignature ) + string.len( metadata ) + 2
    
    if RcString.is( preamble ) and RcString.is( metadata ) and RcString.is( postamble ) then
        -- good
    else
        error( "Missing metadata and wrapper for file: " .. RcLua.toString( filePath ) )
    end
    ...
    if not RcUtils.testMode then
        ...
        local ok, orNot = RcFileUtils.writeFile( filePath, contents )
        return ok, orNot
        
        --[[if RcUtils.debugMode then
            local blocks, qualification = RcJpegUtils.peruseFile( filePath )
            if blocks and not qualification then
                RcUtils.logMessageLine( LOC( "$$$/X=RC Jpeg Metadata updated in ^1", filePath ) )
                return true
            else
...
--[[    
        Synopsis:       Write a jpeg file given components previously read and new or updated rc-jpeg-metadata.
        
        Notes:          Exported photos are always written with fresh source content, and so dont use this function,
        ...
        Notes:          - Metadata format is a black box to this function.
                        - signature is considered sacred: will never change.
--]]
function RcJpegUtils.reWriteFileWithRcMetadata( filePath, preamble, metadata, postamble )
	return RcJpegUtils.reWriteFileWithMetadata( filePath, preamble, metadata, postamble, rcMetadataSignature, rcBlockType )
end
...
--]]
function RcJpegUtils.generateContentWithXmpMetadata( preamble, settingsBlockHeader, settingsPreamble, settings, settingsPostamble, postamble )
	...
	-- _debugTrace( "blklen", "blklen: " .. RcLua.toString( blklen ) )
	...
	assert( len2 >= 0 and len2 < 256, "_len2: " .. RcLua.toString( len2 ) )
	assert( len1 >= 0 and len1 < 256, "_len1: " .. RcLua.toString( len1 ) )
	-- _debugTrace( "xlen12", "len1: " .. RcLua.toString( len1 ) .. ", len2: " .. RcLua.toString( len2 ) )
	local first, second
	first = string.sub( settingsPreamble, 1, 1000 )
	second = string.sub( settingsPostamble, 1, 1000 )
	-- _debugTrace( "xlen12", "len1: " .. RcLua.toString( len1 ) .. ", len2: " .. RcLua.toString( len2 ) )
	-- _debugTrace( "xp1", "first: " .. RcLua.toString( first ) )
	-- _debugTrace( "xps", "sets: " .. RcLua.toString( settings ) )
	-- _debugTrace( "xp2", "second: " .. RcLua.toString( second ) )
	...
	-- _debugTrace( "xblk", "blk: " .. RcLua.toString( blk ) )
	
	-- local contents = LOC( "$$$/X=^1^2^3", preamble, blk, postamble )
	local contents = preamble .. blk .. postamble
	return contents
end
function RcJpegUtils.reWriteFileWithXmpMetadata( filePath, preamble, settingsBlockHeader, settingsPreamble, settings, settingsPostamble, postamble )
	local contents = RcJpegUtils.generateContentWithXmpMetadata( preamble, settingsBlockHeader, settingsPreamble, settings, settingsPostamble, postamble )
	if contents then
	    return RcFileUtils.writeFile( filePath, contents )
	else
	    return false, "block overflow" -- ###2 one serious potential problem of this technique - fix if adobe no dev set support in lr3.
	end
end
...
function RcJpegUtils.writeFileWithMetadata( filePath, contents, metadata, metadataSignature, blockType )
    local start, stop = string.find( contents, string.char( 0xFF ) .. string.char( 0xD8 ), 1, true ) -- SOI
    ...
    return RcJpegUtils.reWriteFileWithMetadata( filePath, preamble, metadata, postamble, metadataSignature, blockType )
end
...
--]]
function RcJpegUtils.writeFileWithRcMetadata( filePath, contents, metadata )
    return RcJpegUtils.writeFileWithMetadata( filePath, contents, metadata, rcMetadataSignature, rcBlockType )
end
...
--[[
        ...
        Instructions:   Remove app1 blocks from source & target contents, then compare the results
                        to decide if there are any meaningful differences.
--]]
function RcJpegUtils.removeApp1Blocks( contents )
    local len1, len2
    local startAt = 1
    local app1 = string.char( 0xFF ) .. string.char( 0xE1 )
    local stringBuffer = {}
...
--] ]
function RcJpegUtils.peruseFile( filePath )
    assert( RcFileUtils.existsAsFile( filePath ), "no jpeg file to read." )
    ...
    while (not done) do
        ...
        repeat -- once
            ...
            rawCount = rawCount + 1
            -- logMessageLine( LOC( "$$$/X=^1: ^2", totalBytesRead, byte ) )
            totalBytesRead = totalBytesRead + 1
            if byte ~= 0xFF then
                -- logMessageLine( "Not FF" )
                -- store in raw block
                break
            end
            ...
            ------------------------
            if marker == SOI then
                logMessageLine( LOC( "$$$/X=^1: SOI", totalBytesRead ) )
                length = 0
                originalLength = 0
            elseif marker == DRI then
                length =_getDoubleByteNumber( file )
                originalLength = length
                assert( length == 4, LOC( "$$$/X=Invalid DRI length: ^1", length ) )
                originalLength = length
                aword = _getDoubleByteNumber( file )
                if RcUtils.debugMode then
                    logMessageLine( LOC("$$$/X=DRI (length ^1)\n", length) )
                    logMessageLine( LOC("$$$/X=^1 restart interval ^2 MCUs\n", prefix, aword) )
                end
            ...
            elseif marker >= APP and marker <= (APP + 15) then
                ...
                if marker == APP and length >= 14 then -- jfif?
                    ...
                    if signature == "JFIF" then
                        ...
                        aword = _getDoubleByteNumber( file )
                        byte = _getByte( file )
                        dpiX=_getDoubleByteNumber( file )
                        dpiY=_getDoubleByteNumber( file )
                        thumbX= _getByte( file )
                        thumbY= _getByte( file )
                        totalBytesRead = totalBytesRead + 9
                        length = length - 9
                        if RcUtils.debugMode then
                            logMessageLine( LOC("$$$/X=^1  JFIF version ^2, ", prefix, aword ) )
                        end
                    ...
                    elseif signature == "JFXX" then
                        ...
                        local thumbX, thumbY
                        local thumbBytes
                        extension=_getByte( file )
                        totalBytesRead = totalBytesRead + 1
                        length = length - 1
                        if extension == 0x10 then
                            if RcUtils.debugMode then
                                logMessageLine( LOC("$$$/X=^1  JFIF extension JPEG thumbnail\n", prefix ) )
                            end
                            while length >0 do
                                dummy = _getByte( file )
                                totalBytesRead = totalBytesRead + 1
                        ...
                        elseif extension == 0x11 then
                            if RcUtils.debugMode then
                                logMessageLine( LOC("$$$/X=^1  JFIF extension 1 byte/pixel thumbnail\n", prefix ) )
                            end
                            thumbX= _getByte( file )
                            thumbY= _getByte( file )
                            totalBytesRead = totalBytesRead + 2
                            length = length -2
                            if RcUtils.debugMode then
                                logMessageLine( LOC("$$$/X=^1    thumbnail size ^2 x ^3\n", prefix, thumbX, thumbY ) )
                            end
                            if RcUtils.debugMode then
                                logMessageLine( LOC("$$$/X=^1    palette:\n", prefix ) )
                            end
                            for i = 0, 256, 1 do
                        ...
                        elseif extension == 0x13 then
                            if RcUtils.debugMode then
                                logMessageLine( LOC("$$$/X=^1  JFIF extension 3 byte/pixel thumbnail\n", prefix ) )
                            end
                            thumbX= _getByte( file )
                            thumbY= _getByte( file )
                            totalBytesRead = totalBytesRead + 2
                            ...
                            if RcUtils.debugMode then
                                logMessageLine( LOC("$$$/X=^1  JFIF extension $^2\n", prefix, extension ) )
                            end
                        end
            ...
            elseif marker == COM then
                ...
                originalLength = length
                if RcUtils.debugMode then
                    logMessageLine( LOC("$$$/X=COM (length ^1)\n  ", length ) )
                end
                totalBytesRead = totalBytesRead + 2
                length = length -2
                while length > 0 do
            ...
            elseif (marker >= SOF and marker <= (SOF + 15)) or (marker == DHP) then
                ...
                originalLength = length
                if RcUtils.debugMode then
                   logMessageLine( LOC("$$$/X=^1 (length ^2)\n", "SOF or DHP", length ) )
                end
                totalBytesRead = totalBytesRead + 2
                length = length -2
                byte = _getByte( file )
                height = _getDoubleByteNumber( file )
                aword=_getDoubleByteNumber( file )
                byte2 = _getByte( file )
                totalBytesRead = totalBytesRead + 6
                length = length - 6
                if RcUtils.debugMode then
                    logMessageLine( LOC("$$$/X=^1  sample precision ^2\n", prefix, byte ) )
                    logMessageLine( LOC("$$$/X=^1  width ^2, height ^3  components ^4\n", prefix, aword, height, byte2 ) )
                end
                while byte2 > 0 do
                    byte2 = byte2 - 1
                    dummy=_getByte( file )
            ...
            elseif marker == SOS then
                ...
                originalLength = length
                if RcUtils.debugMode then
                    logMessageLine( LOC("$$$/X=SOS (length ^1)\n", length ) )
                end
                totalBytesRead = totalBytesRead + 2
                length = length -2
                byte2 = _getByte( file )
                ...
                length = length - 1
                if RcUtils.debugMode then
                    logMessageLine( LOC("$$$/X=^1  components ^2\n", prefix, byte2 ) )
                end
                while byte2 > 0 do
                    byte2 = byte2 - 1
                    dummy=_getByte( file )
            ...
            elseif marker == DQT then
                ...
                originalLength = length
                if RcUtils.debugMode then
                    logMessageLine( LOC("$$$/X=DQT (length ^1)\n", length ) )
                end
                totalBytesRead = totalBytesRead + 2
                length = length -2
                while length > 0 do
            ...
            elseif marker == DHT then
                ...
                originalLength = length
                if RcUtils.debugMode then
                    logMessageLine( LOC("$$$/X=DHT (length ^1)\n", length ) )
                end
                totalBytesRead = totalBytesRead + 2
                length = length -2
                while length > 0 do
                ...
                if RcUtils.debugMode then
                    logMessageLine( LOC("$$$/X=^1  table ^2\n", prefix, byte ) )
                end
                for i=0, i<16, 1 do
                    huff[i]= _getByte( file )
                end
                totalBytesRead = totalBytesRead + 16
                length = length - 16
                for i=0, i < 16, 1 do
                    if RcUtils.debugMode then
                        logMessageLine( LOC("$$$/X=^1    bits ^2 (codes=^3) ", prefix, i+1, huff[i] ) )
                    end
                    while huff[i] > 0 do
                        huff[i] = huff[i] - 1
                        byte2 = _getByte( file )
            ...
            elseif marker == DAC then
                ...
                originalLength = length
                if RcUtils.debugMode then
                    logMessageLine( LOC("$$$/X=DAT (length ^1)\n", length ) )
                end
                totalBytesRead = totalBytesRead + 2
                length = length -2
                while length > 0 do
                    ...
                    length = length -2
                    if RcUtils.debugMode then
                        logMessageLine( LOC("$$$/X=^1  id ^2 conditioning ^3\n", prefix, byte, byte2 ) )
                    end
                end
            elseif marker >= RST and marker <= (RST + 7) then
                if RcUtils.debugMode then
                    logMessageLine( LOC("$$$/X=RST^1\n", marker - RST ) )
                end
                length = 0
                originalLength = 0
            elseif marker == DNL then
                ...
                length = length -4
                if RcUtils.debugMode then
                    logMessageLine( LOC("$$$/X=DNL (length ^1)\n", length ) )
                    logMessageLine( LOC("$$$/X=^1  lines ^2\n", prefix, aword ) )
                end
            elseif marker == EOI then
                if RcUtils.debugMode then
                    logMessageLine( LOC("$$$/X=EOI\n") )
                end
                length = 0
                originalLength = 0
            elseif marker == EXP then
                ...
                originalLength = length
                if RcUtils.debugMode then
                    logMessageLine( LOC("$$$/X=DHP (length ^1)\n", length ) )
                end
                byte = _getByte( file )
                totalBytesRead = totalBytesRead + 3
                length = length -3
            elseif marker == TEM then
                ...
                originalLength = length
                if RcUtils.debugMode then
                    logMessageLine( LOC("$$$/X=marker $^1 (length ^2)\n", byte, length ) )
                end
                totalBytesRead = totalBytesRead + 2
                length = length -2
                if RcUtils.debugMode then
            ...
            if length > 0 then -- residual length
                if RcUtils.debugMode then
                    logMessageLine( LOC("$$$/X=^1  bad length (residual=^2)\n", prefix, length ) )
                end
    ...
    logMessageLine( LOC( "$$$/X=Total bytes read: ^1/^2", totalBytesRead, fileSize ) )
    file:close()
...
--[[    *** SAVE FOR POSSIBLE FUTURE RESURRECTION. For now, calling context does the work of this function.
        ...
        Synopsis:       Read contents, and if rc-metadata present, removes it before returning the contents.
        ...
        Returns:        - contents, nil:        worked - contents sans rc-metadata.
                        - nil, error-message:   no-go.
--] ]
function RcJpegUtils.readFileSansRcMetadata( filePath )
    local preamble, metadata, postamble, qualification = RcJpegUtils.readFileWithRcMetadata( filePath )
    if preamble then
...
--] ]
function RcJpegUtils.removeRenderDate( contents )
